...
double vPivotLine; // Pivot Line Price 
   ...
   double LastFourHourlyHigh;
   double LastFourHourlyLow;
   double LastFourHourlyOpen;
   double FourHourlyOpen;
   double FourHour_Price[][6];
   double Pivot,S1,S2,S3,S4,R1,R2,R3,R4;
...
/*----------------- mulai perhitunga pivot H4 -------------- */
   ArrayCopyRates(FourHour_Price,(Symbol()), 240);
   LastFourHourlyHigh  = FourHour_Price[1][3];
   LastFourHourlyLow   = FourHour_Price[1][2];
   LastFourHourlyOpen = FourHour_Price[1][1];
   FourHourlyOpen = FourHour_Price[0][1];
   
   Pivot = ((LastFourHourlyHigh + LastFourHourlyLow + LastFourHourlyOpen + FourHourlyOpen +(FourHourlyOpen - (1*Point)))/5);

   R1 = (2*Pivot)-LastFourHourlyLow;
   S1 = (2*Pivot)-LastFourHourlyHigh;

   R2 = Pivot+(R1-S1);
   S2 = Pivot-(R1-S1);
   
   R3 = Pivot+(R2-S2); //(LastFourHourlyHigh + (2*(Pivot-LastFourHourlyLow)));
   S3 = Pivot-(R2-S2); //(LastFourHourlyLow - (2*(LastFourHourlyHigh-Pivot)));  
...
/*
   ...
   BuyLine3 = NormalizeDouble(ObjectGet("H4_S3_Line",OBJPROP_PRICE1),Digits);
   SellLine3 = NormalizeDouble(ObjectGet("H4_R3_Line",OBJPROP_PRICE1),Digits);
   BuyLine4 = NormalizeDouble(ObjectGet("H4_S4_Line",OBJPROP_PRICE1),Digits);
   SellLine4 = NormalizeDouble(ObjectGet("H4_R4_Line",OBJPROP_PRICE1),Digits);
      ...
      if(Orderterbuka==0 && GF_X() && GF_X2() && Stop_BUY())
        {
         ...
         int Ord_Send=OrderSend(Symbol(),0,Lot,Ask-Spread,slippage,SLB,0,EA_NAME,ExpertID,0,clrLime);
        }
      //-----------------------------------------------------------------------------------------||
      ...
      if(Orderterbuka==0 && GF_Y() && GF_Y2() && Stop_SELL())
        {
         ...
         int Ord_Send=OrderSend(Symbol(),1,Lot,Bid-Spread,slippage,SLS,0,EA_NAME,ExpertID,0,clrRed);
        }
     }
//-----------------------------------------------------------------------------------------||
